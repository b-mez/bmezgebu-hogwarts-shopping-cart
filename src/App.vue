                    ...
                    <div class="cart-list-item" v-for="item in shoppingCartItems" :key="item.id">
                        ...
                        <div class="item-details-with-actions">
                            <div class="item-details">
                                ...
                                <p class="in-stock-status" v-if="item.isInStock"> <i class="fa-solid fa-check"></i> In
                                    stock</p>
                                <p class="on-backorder-status" v-else="item.isInStock"> <i
                                        class="fa-solid fa-hourglass-half"></i> On backorder</p>
                            </div>
...
function increaseOne(id) {
    ...
    if (item) {
        item.quantity = Number(item.quantity) + 1; // Ensures quantity is a number
    }
}


function removeItem(id) {
    // Step 1: Find index of the item to remove
    let index = shoppingCartItems.value.findIndex(item => { return item.id == id })
